...
export default function CSTeam() {
  ...
  const [templates, setTemplates] = useState([
    { id: 'intro', name: 'Say Hello', status: 'empty', config: null, previewData: null, savedData: null },
    { id: 'demo', name: 'Walkthrough', status: 'empty', config: null, previewData: null, savedData: null },
    { id: 'closing', name: 'Ending', status: 'empty', config: null, previewData: null, savedData: null }
  ]);
  ...
  const handleRenderFinal = async () => {
    ...
    track('create-video-clicked', { template: 'cs-team' });
    ...
    try {
      ...
      for (const template of readyTemplates) {
        ...
        // Flag to omit QR banner for this template
        if (['intro', 'closing'].includes(template.id)) {
          formData.append('omit_qr', 'true');
        }
      ...
      // Template order: 1=intro, 5=demo, 6=closing (other slots unused)
      const templateOrderSlots = [1, 5, 6];
      templateOrderSlots.forEach((templateOrder, index) => {
        const video = videoMap[templateOrder];
        
        console.log(`Looking for template order ${templateOrder} in slot ${index}:`, video ? `Found ${video.templateName}` : 'Not found');
        
        if (video) {
          const videoBlob = new Blob([video.videoData], { type: 'video/mp4' });
          concatenateFormData.append(`segment_${index}`, videoBlob, `segment_${index}.mp4`);
          concatenateFormData.append(`order_${index}`, video.order.toString());
          concatenateFormData.append(`template_id_${index}`, video.templateId || '');
          console.log(`Adding video ${video.templateName} to segment_${index} with order ${video.order}`);
        } else {
          console.log(`No video found for template order ${templateOrder}, skipping segment_${index}`);
        }
      });
  ...
  return (
    <div className="video-builder cs-team-builder">
